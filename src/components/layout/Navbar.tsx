      ...
      <Container>
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center justify-between h-16 gap-4">
            ...
            {/* Left: Logo - Fixed width to prevent layout shift */}
            <div className="w-10">
            ...
            {/* Center: Tabs - Horizontal on all screens */}
            <div className="flex flex-1 justify-center items-center gap-4 sm:gap-8">
              {tabs.map((tab) => (
                <Link
                  key={tab.id}
                  href={tab.href}
                  className={`relative transition-colors py-2 text-sm sm:text-base group ${
                    isActive(tab.href)
